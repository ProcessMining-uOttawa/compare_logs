...
def get_variants(log):
    ...
    # print("# unique variants:", len(variants))
...
def get_variant_ratio(log, vars_stats, print_summ=True):
    num_traces = len(log['case:concept:name'].unique())
    num_vars = vars_stats.shape[0]
    var_ratio = round((num_vars / num_traces) * 100, 2)
    if print_summ:
        print(f"# traces = {num_traces}, # vars = {num_vars}, ratio = {var_ratio}")
    return var_ratio

def get_variants_stats(log):
    variants = pm4py.get_variants(log)
...
class Variant:
    ...
    def __init__(self, nr, cov_amt, cov_perc, cov_perc_cumul, sequence):
        self.nr = nr
        self.cov_amt = cov_amt
        self.cov_perc = cov_perc
        self.cov_perc_cumul = cov_perc_cumul
    ...
    def to_trace(self):
        return Trace([ {'concept:name': activ} for activ in self.sequence ], attributes={ 'concept:name': self.nr })    
    
    def __repr__(self):
        return self.__str__()
    def __str__(self):
        return f"{self.nr}: # {self.cov_amt} (+ {self.cov_perc}% ->{self.cov_perc_cumul})"