...
def logs_to_dfgs(logs):
    ...
    for index, log in enumerate(logs):
        ...
        dfg.columns = [ f"{col}_{index}" if (col != 'src' and col != 'tgt') else col.lower() for col in dfg.columns ]
        dfgs.append(dfg)
...
def merge_dfgs(dfgs):
    for i, dfg in enumerate(dfgs):
        dfg.rename(columns={'freq': f'freq_{i}'}, inplace=True)
...
def compare_nodes_edges_logs(logs):
    dfgs = logs_to_dfgs(logs)
    merge = merge_dfgs(dfgs)
    
    return __compare_nodes_edges__(dfgs, merge)

def compare_nodes_edges_dfg(dfgs):
    # macho macho code
    dfgs = [ pd.DataFrame([ [ key[0].lower(), key[1].lower(), value ] for key, value in dfg.items() ], columns=[ 'src', 'tgt', 'freq' ]) for dfg in dfgs ]
    merge = merge_dfgs(dfgs)
    
    return __compare_nodes_edges__(dfgs, merge)

def __compare_nodes_edges__(dfgs, merge):
    node_diff = []
    edge_diff = []
    ...
    return [ len(dfgs), node_diff, edge_diff ]


def print_compare_results(cmp_results, log_labels=None, group_type=GroupTypes.BY_ELEMENT):
    num_dfgs, node_diff, edge_diff = cmp_results
    ...
    if group_type == GroupTypes.BY_ELEMENT:
        ...
        # for each DFG j
        for i in range(num_dfgs):
            print(f"> log {label_fn(i)}")

            # for each DFG j
            for j in range(num_dfgs):
                if i == j:
                    continue
...
def plot_metrics_logs(logs, log_labels=None, metric1=None, metric2=None, normalize=None, time_unit=None, per_edge=False, edges=None):
    _, axes = plt.subplots(nrows=1, ncols=2, figsize=(12, 5))
    plot_metric_logs(logs, log_labels, metric1, normalize, time_unit, per_edge, edges, subplot_of=axes[0])
    plot_metric_logs(logs, log_labels, metric2, normalize, time_unit, per_edge, edges, subplot_of=axes[1])

# normalize: True/False
# metric: 'freq', 'time_mean', 'time_median', 'time_min', 'time_max', 'time_stdev'
# time_unit: min, hr, day, month, year
def plot_metric_logs(logs, log_labels=None, metric='freq', normalize=None, time_unit=None, per_edge=False, edges=None, subplot_of=None):
    dfgs = logs_to_dfgs(logs)
    merge = merge_dfgs(dfgs)
    ...
    def do_plot(df, title, ylabel, x=None, legend=True):
        if subplot_of is not None:
            df.plot.barh(ylabel=ylabel, x=x, legend=legend, subplots=True, ax=subplot_of)
            subplot_of.title.set_text(title)
        else:
            df.plot.barh(title='Comparison between Application Types (Initial/New) within Event Pairs\n Reallocation Analysis\n Period I: July 6, 2022 – October 31, 2022  & Period II: January 1, 2023 – May 31, 2023\n Bar lines represent "Median Time" in month', ylabel=ylabel, x=x, legend=legend, figsize=(15, 25))
